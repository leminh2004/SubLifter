...
- **Khung quét phụ đề cố định (Optimal Crop Box)**: Cố định ngầm khung check phụ đề tối ưu nhất cho hardsub: quét toàn bộ chiều ngang (để đảm bảo không bị mất chữ ở hai biên của dòng sub dài) và 40% chia đều ở 2 đầu chiều dọc (Top 20% và Bottom 20% xếp chồng đứng để nhận diện đồng thời cả sub trên và dưới cực nhanh).
...
2. Cài đặt các thư viện phụ thuộc:
   ...
   *Lưu ý 1 (GPU PyTorch cho EasyOCR): Để kích hoạt tăng tốc GPU (CUDA) giúp trích xuất siêu tốc bằng EasyOCR, hãy chạy lệnh cài đặt PyTorch GPU chuyên dụng:*
   ```bash
   # Dành cho CUDA 12.1 / 12.4
   pip install torch torchvision --index-url https://download.pytorch.org/whl/cu124 --force-reinstall
   ```
   
   *Lưu ý 2 (Cho PaddleOCR - Khuyên dùng): Để sử dụng công cụ PaddleOCR (nhận diện tiếng Việt chuẩn xác hơn EasyOCR), hãy cài đặt thêm:*
   ```bash
   # Cài đặt wrapper PaddleOCR
   pip install paddleocr
   
   # Cài đặt Core PaddlePaddle (chọn 1 trong 2 bản tùy card đồ họa của bạn):
   # Bản chạy CPU:
...
2. Kiểm tra **Ảnh xem trước** ở khung bên phải: Hai khung màu đỏ thể hiện chính xác 2 vùng quét cố định (Biên trên 20% và Biên dưới 20%, quét toàn bộ chiều ngang) để bạn quan sát trực quan.
...
* `--engine` (Tùy chọn): Công cụ OCR sử dụng, chọn `paddle` hoặc `easyocr` (mặc định: `paddle`).
* `--no-spellcheck` (Tùy chọn): Tắt tính năng tự động sửa lỗi chính tả và cách từ.
* `--sample-rate` (Tùy chọn): Tốc độ quét (số khung hình xử lý mỗi giây, mặc định: `4.0`).
* `--ymin`, `--ymax`, `--xmin`, `--xmax` (Tùy chọn): Tọa độ cắt thủ công từ `0.0` đến `1.0` (mặc định quét 2 vùng: top 20% và bottom 20%, ngang 100% full width).
